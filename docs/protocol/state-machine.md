...
stateDiagram-v2
  ...
  FUNDED --> REFUNDED: refundAfterDeadline / no valid proof
...
| `submitProofBundle` | Seller | State is `FUNDED`; proof hash and URI are non-empty; task is within execution window. | Stores proof bundle data and moves to `PROOF_SUBMITTED`. |
...
| `refundAfterDeadline` | Buyer or anyone | State is `FUNDED`; deadline has passed; no proof bundle was accepted. | Refunds escrow and moves to `REFUNDED`. |
...
| `EXPIRED` | Contract state is `INTENT_CREATED` or `COMMITMENT_SUBMITTED`, deadline or acceptance window has passed, and no funds are escrowed. |
