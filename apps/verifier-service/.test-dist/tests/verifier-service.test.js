...
import { buildCallIntentHash, buildVerificationReportTypedData, hashExecutionCommitment, hashObject } from "@fulfillpay/sdk-core";
import { MemoryStorageAdapter } from "@fulfillpay/storage-adapter";
import { MockZkTlsAdapter } from "@fulfillpay/zktls-adapter";
import { Wallet, verifyTypedData } from "ethers";
import { buildProofConsumptionKeys, CentralizedVerifier, InMemoryProofConsumptionRegistry, PrismaProofConsumptionRegistry, REQUIRED_VERIFICATION_CHECKS, VerifierInputIntegrityError, VerifierInputUnavailableError, createVerifierHttpServer, signVerificationReport, toSettlementReportStruct } from "../src/index.js";
...
const OTHER_TASK_ID = "0x8888888888888888888888888888888888888888888888888888888888888888";
const OTHER_PROOF_BUNDLE_HASH = "0x9999999999999999999999999999999999999999999999999999999999999999";
const WRONG_COMMITMENT_HASH = "0xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
const WRONG_CALL_INTENT_HASH = "0xcccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc";
...
class MockSettlementReader {
    task;
    verifierAuthorized;
    proofSubmissionGracePeriod;
    verificationTimeout;
    proofBundleConsumed = false;
    constructor(task, verifierAuthorized = true, proofSubmissionGracePeriod = "600000", verificationTimeout = "600000") {
        this.task = task;
        this.verifierAuthorized = verifierAuthorized;
        this.proofSubmissionGracePeriod = proofSubmissionGracePeriod;
    ...
    async isVerifierAuthorized(_verifier) {
...
class FakeProofConsumptionKeyDelegate {
    ...
    async findFirst(args) {
        return (this.records.find((record) => args.where.OR.some((candidate) => candidate.keyType === record.keyType && candidate.key === record.key)) ?? null);
    }
...
class FakePrismaClient {
    proofConsumptionKey = new FakeProofConsumptionKeyDelegate();
}
test("signs a passing verification report for contract settlement", async () => {
    ...
    const recovered = verifyTypedData(typedData.domain, typedData.types, typedData.message, result.report.signature).toLowerCase();
...
test("rejects signing reports whose passed flag does not match required checks", async () => {
    ...
    await assert.rejects(() => signVerificationReport({
        ...unsignedReport,
        checks: {
        ...
        passed: true
    }, fixture.verifierWallet), /passed must equal the AND/);
});
test("rejects signing reports missing required checks", async () => {
    const fixture = await buildFixture();
    const result = await fixture.verifier.verifyTask(fixture.task.taskId, { markProofsConsumed: false });
    const { signature: _signature, reportHash: _reportHash, ...unsignedReport } = result.report;
    const checks = { ...unsignedReport.checks };
    delete checks.modelMatched;
    await assert.rejects(() => signVerificationReport({
        ...unsignedReport,
        checks
    }, fixture.verifierWallet), /checks.modelMatched is required/);
});
...
test("rejects report generation when the proof bundle was already consumed on-chain", async () => {
    ...
    await assert.rejects(() => fixture.verifier.verifyTask(fixture.task.taskId, { markProofsConsumed: false }), /already consumed by the settlement contract/);
});
...
test("rejects when the commitment assigns a different verifier", async () => {
    ...
    await assert.rejects(() => fixture.verifier.verifyTask(fixture.task.taskId, { markProofsConsumed: false }), /not the verifier assigned/);
});
test("rejects when the signer is not registry authorized", async () => {
    const fixture = await buildFixture({ verifierAuthorized: false });
    await assert.rejects(() => fixture.verifier.verifyTask(fixture.task.taskId, { markProofsConsumed: false }), /not authorized/);
});
test("rejects when the configured report verifier differs from the signer", async () => {
    ...
    await assert.rejects(() => fixture.verifier.verifyTask(fixture.task.taskId, { markProofsConsumed: false }), /does not match signer/);
});
test("rejects when proof submission is outside the configured grace period", async () => {
    ...
    await assert.rejects(() => fixture.verifier.verifyTask(fixture.task.taskId, { markProofsConsumed: false }), /outside the configured grace period/);
});
test("rejects storage hash mismatches as integrity failures", async () => {
    ...
    await assert.rejects(() => fixture.verifier.verifyTask(fixture.task.taskId, { markProofsConsumed: false }), (error) => error instanceof VerifierInputIntegrityError && !(error instanceof VerifierInputUnavailableError));
});
test("rejects when report generation is after the verification timeout", async () => {
    ...
    await assert.rejects(() => fixture.verifier.verifyTask(fixture.task.taskId, { markProofsConsumed: false }), /verification timeout expired/);
});
test("persists consumed proof keys through the Prisma registry adapter", async () => {
    ...
    const record = {
        ...
        reportHash: "0x8888888888888888888888888888888888888888888888888888888888888888",
        passed: true,
        verifiedAt: VERIFIED_AT
    };
...
async function buildFixture(options = {}) {
    ...
    const settlement = new MockSettlementReader(task, options.verifierAuthorized ?? true, options.proofSubmissionGracePeriod, options.verificationTimeout);
