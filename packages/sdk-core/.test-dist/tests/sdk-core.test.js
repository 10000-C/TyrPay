...
import { buildCallIntent, buildCallIntentHash, buildTaskContext, buildTaskContextHash, buildVerificationReportTypedData, canonicalize, hashCallIntent, hashDeliveryReceipt, hashExecutionCommitment, hashProofBundle, hashTaskContext, hashTaskIntent, hashVerificationReport, hashVerificationReportStruct, hashVerificationReportTypedData } from "../src/index.js";
...
test("fixture canonical strings and hashes match sdk-core helpers", () => {
    const taskIntent = loadFixture("test/fixtures/protocol/task-intents/task-intent.basic.json");
    const taskContext = loadFixture("test/fixtures/protocol/task-contexts/task-context.basic.json");
    const commitment = loadFixture("test/fixtures/protocol/commitments/commitment.openai-compatible.json");
    const callIntent = loadFixture("test/fixtures/protocol/call-intents/call-intent.basic.json");
    const receipt = loadFixture("test/fixtures/protocol/receipts/receipt.mock.valid.json");
    const proofBundle = loadFixture("test/fixtures/protocol/proof-bundles/proof-bundle.pass-basic.json");
    const verificationReport = loadFixture("test/fixtures/protocol/verification-reports/verification-report.pass-basic.unsigned.json");
    const vector = loadJson("test/vectors/hashing/pass-basic.json");
...
test("task context and call intent builders reproduce the fixture vectors", () => {
    const taskContextFixture = loadFixture("test/fixtures/protocol/task-contexts/task-context.basic.json");
    const callIntentFixture = loadFixture("test/fixtures/protocol/call-intents/call-intent.basic.json");
    ...
    assert.equal(buildCallIntentHash({
...
test("verification report hash ignores reportHash and signature fields", () => {
    const reportFixture = loadFixture("test/fixtures/protocol/verification-reports/verification-report.pass-basic.unsigned.json");
...
test("proof bundle validation rejects duplicate call indices and mismatched aggregate usage", () => {
    const proofBundleFixture = loadFixture("test/fixtures/protocol/proof-bundles/proof-bundle.pass-basic.json");
...
test("verification report helpers reject inconsistent settlement actions", () => {
    ...
    assert.throws(() => buildVerificationReportTypedData(invalidReport), /settlement\.action must be REFUND when UnsignedVerificationReport\.passed is false/);
});
test("verification report helpers reject mismatched reportHash decorations", () => {
    ...
    assert.throws(() => buildVerificationReportTypedData(invalidReport), /reportHash does not match the canonical unsigned report hash/);
});
test("eip712 helpers reproduce the verification report vector", () => {
    const reportFixture = loadFixture("test/fixtures/protocol/verification-reports/verification-report.pass-basic.unsigned.json");
    const vector = loadJson("test/vectors/eip712/verification-report-pass-basic.json");
